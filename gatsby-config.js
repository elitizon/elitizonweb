const siteMetadata = {
  ...
  description: `We're reimagining products and services by leveraging data and AI to gain insights that drive unprecedented innovation. Our relentless focus on these technologies allows us to continuously revolutionize products in extraordinary new ways and reshape industries through the power of data and AI..`,
        ...
        feeds: [
          {
            serialize: ({ query: { site, allMdx } }) => {
              return allMdx.edges.map((edge) => {
                return Object.assign({}, edge.node.frontmatter, {
                  ...
                  custom_elements: [
                    { "content:unencoded": `${edge.node.rawBody}` }
                  ]
                })
              })
            },
            query: `
            {
              allMdx(sort: {order: DESC, fields: [frontmatter___date]}, filter: {frontmatter: {published: {eq: true}, posttype: {eq: "blog"}}}) {
                edges {
                  node {
                    excerpt
                    rawBody
